...
client.on("message", async message => {
  ...
  if(command === "help") {
       message.channel.sendMessage("```The prefix is: f: | User commands: ping, help, staff, updates     Staff commands: kick, ban, purge [DISABLED], say```")
  }
  if(command === "ping") {
  ...
  if(command === "staff"){
    message.channel.sendMessage("```Bot Owner & Host: darkline1 | Web Master & Owner: grifkuba.net | Owner: darkline1 | Co Owner: WavyTaeTae | Administrator: xXAmazingTuku, 2blocky2 | Moderator: Renewable123``` Not listed? Contact darkline1#9081 to be added!")
  }
  
  if(command === "updates"){
    message.channel.sendMessage ("``11/17/17``**:** Commands added to the bot. Prefix is ``f:``. ``purge`` removed.")
  }
